...
describe('RFC-013 - integration au pipeline', () => {
  ...
  test('SCENE_ET_PARENT ne lance aucun pipeline pour un participant d une sous-scene soeur active', async () => {
    const compteur = { appels: 0 }
    ...
    const resultat = await traiterInteraction(
      fabriqueSollicitation({
        ...
        evenement: fabriqueEvenement({
          destinataireIds: ['a', 'b', 'c', 'd'],
          sceneId: 'principale',
          sousSceneId: 'privee',
          politiqueDiffusion: 'scene_et_parent',
        }),
      }),
      contexte({
        participants: {
