...
function allouerIntentions(candidates, ressourcesDisponibles) {
  const retenues = []
  ...
  return { retenues, ecartes, conflits }
}
...
export function resoudreConflitsActions({
  ...
  const invalidesParDependance = new Map()
  let allocation
  while (true) {
    allocation = allouerIntentions(
      candidates.filter(item => !invalidesParDependance.has(item.id)),
      ressourcesDisponibles,
    )
    const idsRetenus = new Set(allocation.retenues.map(item => item.id))
    const nouvellementInvalides = allocation.retenues.filter(intention =>
      (contraintes(intention).idsIntentionsRequises ?? []).some(id => !idsRetenus.has(id)))
    if (nouvellementInvalides.length === 0) break
    for (const intention of nouvellementInvalides) {
      const requiseAbsente = (contraintes(intention).idsIntentionsRequises ?? [])
        .find(id => !idsRetenus.has(id))
      ...
      invalidesParDependance.set(intention.id, { intention, conflit })
    }
  }

  const { retenues, ecartes, conflits } = allocation
  for (const { intention, conflit } of invalidesParDependance.values()) {
    ajouterEcart(ecartes, conflits, intention, conflit)
  }
