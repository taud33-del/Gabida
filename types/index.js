/**
 * types/index.js
 *
 * Langage interne de Gabida — Data Models v1.0
 * Point d'entrée unique. Ce fichier ne définit rien : il réexporte tout.
 *
 * Règles fondamentales :
 *   - Les types sont indépendants des modules, des fiches et des providers.
 *   - Aucun type ne contient de logique.
 *   - Tous les modules importent depuis ici — aucun module n'est propriétaire d'un type de communication.
 *   - Les types métier propres à un domaine (Souvenir, MemoireVecue...) restent dans leur module.
 *
 * Pour importer dans un module :
 *   import { PlayerMessage } from '../types/index.js'   // ou
 *   import { PlayerMessage } from '../types/PlayerMessage.js'
 */

export * from './PlayerMessage.js'
export * from './Evenement.js'
export * from './FiltreRelationnel.js'
export * from './Ressenti.js'
export * from './Decision.js'
export * from './Prompt.js'
export * from './ReponseIA.js'
export * from './Etat.js'
export * from './Sauvegarde.js'

// ─── Gabida V2 — modèle multi-participants (Phase 1 : contrats de données) ─────
// Contrats JSDoc uniquement (aucun export runtime). Ils coexistent avec les
// types V1 ci-dessus sans les modifier ni les remplacer.
export * from './Participant.js'
export * from './EvenementInteraction.js'
export * from './Sollicitation.js'
export * from './RelationParticipant.js'
export * from './ProvenanceRelation.js'
export * from './TransmissionInformation.js'
export * from './ResultatTransmissionInformation.js'
export * from './IntentionMetier.js'
export * from './PlanificationExecutionParticipant.js'
export * from './ResultatArbitrageIntentionsMetier.js'
export * from './ConflitAction.js'
export * from './RegleResolutionConflit.js'
export * from './ResultatResolutionConflits.js'
export * from './ActionParticipant.js'
export * from './TraceInteraction.js'
export * from './EtatInteraction.js'
export * from './ResultatInteraction.js'
export * from './PerceptionParticipant.js'
export * from './ProvenanceEpistemique.js'
export * from './FaitEpistemique.js'
export * from './RevisionEpistemique.js'
export * from './GroupeParticipants.js'
export * from './ContexteScene.js'
export * from './SceneInteraction.js'
export * from './PresenceParticipantScene.js'
export * from './TransitionScene.js'
export * from './ResultatResolutionScene.js'
