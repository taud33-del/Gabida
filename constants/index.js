/**
 * constants/index.js
 *
 * Constantes officielles de Gabida — point d'entrée unique.
 * Ce fichier ne définit rien : il réexporte tout.
 *
 * Le moteur ne compare jamais des chaînes libres.
 * Il manipule uniquement les constantes définies ici.
 *
 * Pour importer dans un module :
 *   import { INTENTIONS } from '../constants/index.js'          // ou
 *   import { INTENTIONS } from '../constants/Intentions.js'
 */

export * from './Versions.js'
export * from './Intentions.js'
export * from './MomentsNarratifs.js'
export * from './SourcesInfluence.js'
export * from './TonalitesFiltre.js'
export * from './OriginesRessenti.js'
export * from './Objectifs.js'
export * from './Attitudes.js'
export * from './DirectionsNarratives.js'
export * from './RolesMessage.js'
export * from './TypesSouvenir.js'
export * from './ImportancesMemoire.js'
export * from './Providers.js'
export * from './RolesMessageInterne.js'
export * from './RuntimeStates.js'

// ─── Gabida V2 — modèle multi-participants (Phase 1 : constantes uniquement) ───
export * from './TypesParticipant.js'
export * from './StatutsParticipant.js'
export * from './TypesProfilParticipant.js'
export * from './VisibilitesEvenement.js'
export * from './TypesActionParticipant.js'
export * from './CanauxPerception.js'
export * from './PrecisionsPerception.js'
export * from './TypesFaitEpistemique.js'
export * from './StatutsFaitEpistemique.js'
export * from './TypesProvenanceEpistemique.js'
export * from './OperationsRevisionEpistemique.js'
export * from './StatutsRelationParticipant.js'
export * from './TypesProvenanceRelation.js'
export * from './DimensionsRelationStandard.js'
export * from './ModesMiseAJourRelation.js'
export * from './StatutsTransmissionInformation.js'
export * from './TypesResultatTransmission.js'
export * from './TypesIntentionMetier.js'
export * from './PrioritesIntentionMetier.js'
export * from './StatutsIntentionMetier.js'
export * from './ModesPlanificationExecution.js'
export * from './TypesConflitAction.js'
export * from './StatutsResolutionAction.js'
export * from './StatutsGroupeParticipants.js'
export * from './TypesSceneInteraction.js'
export * from './StatutsSceneInteraction.js'
export * from './PolitiquesDiffusionScene.js'
